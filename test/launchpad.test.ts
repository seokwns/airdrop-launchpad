...
describe("Launchpad", () => {
  ...
  let launchpadAmount = ethers.parseEther("60000");
  ...
  before(async () => {
    ...
    await token.mint(deployer.address, launchpadAmount);
  });
  ...
  it("Should update launchpad amount", async () => {
    launchpadAmount = ethers.parseEther("50000");
    await token.approve(launchpad.target, launchpadAmount);
  ...
  it("Should claim after launchpad starts", async () => {
    ...
    // 앞선 테스트에서 claimRatio 를 200으로 변경했으므로, 10 * 200 / 1 = 2000 이어야 함
  ...
  it("Should get progress", async () => {
    ...
    const expected = ((launchpadAmount - ethers.parseEther("2000")) * BigInt(PERCENT_PRECISION)) / launchpadAmount;
    expect(progress).to.equal(expected);
  });

  it("Should refund if exceed account cap", async () => {
    const value = ethers.parseEther("200");

    await expect(launchpad.connect(tester1).claim({ value }))
      .to.be.emit(launchpad, "Claimed")
  ...
  it("Should refund if claim value is not enough", async () => {
    ...
    const claimAmount = await token.balanceOf(launchpad.target);

    await expect(launchpad.connect(tester2).claim({ value }))
      .to.be.emit(launchpad, "Claimed")
      .to.be.emit(launchpad, "InsufficientChuruRefunded");
  ...
  it("Should payout", async () => {
    await expect(launchpad.payout()).to.be.emit(launchpad, "Payouted");

    const afterBalance = await ethers.provider.getBalance(launchpad.target);
    const afterChuruBalance = await token.balanceOf(launchpad.target);
    expect(afterBalance).to.equal(0);
    expect(afterChuruBalance).to.equal(0);
  });
});
